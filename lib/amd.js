...
async function handleSuccessfulConnection(sock) {
  ...
  try {
    ...
    const gitSyncResult = await scraper.syncgit();
  ...
  try {
    await sock.sendMessage(userId, { text: `\`\`\`${statusMessage.trim()}\`\`\`` }, { disappearingMessagesInChat: true, ephemeralExpiration: 86400 });
  } catch (error) {
...
function handleDisconnection(lastDisconnect) {
  ...
  const reasons = {
    ...
    [DisconnectReason.connectionReplaced]: 'Connection Replaced, Please Close Current Session First',
  ...
  if (statusCode in reasons) {
    console.log(reasons[statusCode]);
  } else {
...
async function MakeSession(sessionId = SESSION_ID, sessionDir = __dirname + '/baileys', useOfficial = false) {
  const sanitizedSessionId = sessionId.replace(/^SESSION_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^SESSION_ID_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^ASTA_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/Astro;;;/gi, "").replace(/Asta;;;/gi, "").replace(/Astropeda;;;/gi, "").trim();
  const sessionPath = `${sessionDir}/${sanitizedSessionId}/auth_info_multi.json`;

  if (fs.existsSync(sessionPath)) {
    try {
      const sessionData = JSON.parse(fs.readFileSync(sessionPath, 'utf8'));
      const sessionAuthState = await useMultiFileAuthState(sessionDir);
      sessionAuthState.state.creds = sessionData.creds;
      sessionAuthState.state.keys = sessionData.keys;
      await sessionAuthState.saveCreds();
    ...
    console.error('No session file found. Please ensure the correct path is provided.');
  }
}
async function main() {
